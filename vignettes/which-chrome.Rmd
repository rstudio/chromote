...
By default, if called without any arguments, `local_chrome_version()` will use `version = "latest-stable"` — the latest stable version available — and `binary = "chrome"` — the standard Chrome browser binary.
If you don't already have a copy of the requested version of the binary, `local_chrome_version()` will download it for you.

For scripts with a longer life span, you may want to pin your script to a specific version of Chrome or `chrome-headless-shell`.
...
chromote stores previously downloaded versions of Chrome so that, once downloaded, you won't need to download the binary again.
You can list all of the versions and binaries you've installed with `chrome_versions_list()`.
...
You can also request all available Chrome versions and binaries with `chrome_versions_list("all")`.
